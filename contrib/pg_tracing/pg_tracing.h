...
typedef struct Span
{
	...
	Size		deparse_info_offset;	/* deparsing info extracted from a
										 * plan node */
...
typedef struct pgTracingTraceContext
{
	...
	bool		in_aborted_transaction; /* Whether we're in an aborted
										 * transaction state */
...
/* pg_tracing_utils.c */
extern int	add_str_to_trace_buffer(StringInfo str_info, const char *str, int str_len);
extern int	add_worker_name_to_trace_buffer(StringInfo str_info, int parallel_worker_number);
