...
begin_span(Span * span, SpanType type, const pgTracingTrace * trace, uint64 parent_id, uint64 query_id, const int64 *start_span, int nested_level)
{
	...
	/* We use duration to temporarily store ns at span start ns */
...
{
	if (span->name_offset != -1 && qbuffer_size > 0 && span->name_offset <= qbuffer_size)
...
{
	if (span->operation_name_offset != -1 && qbuffer_size > 0 && span->operation_name_offset <= qbuffer_size)
