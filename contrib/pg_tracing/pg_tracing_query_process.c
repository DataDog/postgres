...
/*
 * Check if we have a comment that could store SQLComment information at the start or end of the query.
 * Returns 0 if there's possible comment at the begining, 1 at the end and -1 if no comments were detected.
 */
static int
...
extract_trace_context_from_query(pgTracingTraceContext * trace_context, const char *sqlcomment_str, bool is_parameter)
{
	...
	if (!is_parameter)
	{
		int			possible_comment = has_possible_sql_comment(sqlcomment_str);

		if (possible_comment == -1)
			return;
		else if (possible_comment == 0)
		{
			/* We have a possible comment at the begining */
			start_sqlcomment = sqlcomment_str;
		...
		else if (possible_comment == 1)
		{
			...
			start_sqlcomment = strstr(sqlcomment_str, "/*");
	...
	traceid_left = pnstrdup(&traceparent[3], 16);
	trace_context->trace_id.traceid_left = strtou64(traceid_left, &endptr, 16);
	pfree(traceid_left);
	trace_context->trace_id.traceid_right = strtou64(&traceparent[3 + 16], &endptr, 16);
	if (endptr != traceparent + 35 || errno)
		return;
	trace_context->parent_id = strtou64(&traceparent[36], &endptr, 16);
...
/*
 * Normalise query: Comments are removed, Constants are replaced by $x, all tokens are separated by a single space.
